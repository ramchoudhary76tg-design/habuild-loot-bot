...
POLL_INTERVAL = 0.1  # FASTER!
NUM_WORKERS = 50000    # MAX SPEED!

USED_NUMBERS_FILE = "used_numbers.txt"

# ========== 🔥 ALL FIREBASE PANELS (FULL LIST) ==========
ALL_PANELS = [
    # ----- ORIGINAL PANELS -----
...
print(f"✅ Loaded {len(DATABASES)} Firebase panels")
    ...
    while True:
        try:
            ...
            if number_queue and number_queue.qsize() > 0:
                print(f"📊 Queue: {number_queue.qsize()} numbers | Referrals: {looted_count[0]}")
...
async def api_worker(worker_id: int):
    while True:
        try:
            ...
            await asyncio.sleep(0.02)  # FASTER
        except:
            await asyncio.sleep(0.5)
    ...
    while True:
        try:
            ...
            for num, data in pending_habuild.items():
                if current_time - data.get("timestamp", current_time) > 15:
                    expired.append(num)
            for num in expired:
...
def main():
    print("🤖 Starting Habuild Loot Bot...")
    print(f"📊 Total Panels: {len(DATABASES)}")
    print(f"⚡ Workers: {NUM_WORKERS}")
    print(f"🔗 Referral: {HABIT_REF}")
    print(f"📁 Panel List: {list(DATABASES.keys())}")
    ...
    async def post_init(application: Application):
        ...
        try:
            await application.bot.send_message(
                chat_id=ADMIN_ID, 
                text=f"🔥 HABUILD LOOT BOT ONLINE!\n📊 {len(DATABASES)} Panels Loaded\n🔗 Referral: {HABIT_REF}\n⚡ FULL SPEED MODE ACTIVE!"
            )
        except: pass
        ...
        print("🎯 Bot is LOOTING! Watch your referral count grow!")
