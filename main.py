...
ENGINE_CORES = ['ROCKET', 'TURBO', 'NITRO', 'BLAZE', 'STRIKE', 'LIGHTNING', 'THUNDER', 'PHANTOM', 'SHADOW', 'VENOM', 'FALCON', 'VIPER', 'DRAGON']
...
def get_random_headers():
    ...
    headers = {
        ...
        "Origin": "https://portal.habuild.in",  # ✅ FIXED ORIGIN
...
# ========== 📱 LIVE OTP FUNCTIONS ==========
async def send_live_status(phone: str, status: str, emoji: str = "📱"):
    try:
        if _main_app:
            msg = f"{emoji} *{status}*\n📱 `{phone}`"
            await _main_app.bot.send_message(ADMIN_ID, msg, parse_mode='Markdown')
    except: pass

# ========== 🔥 FIXED REGISTRATION - WITH PROPER ORIGIN ==========
async def trigger_registration(phone_10d: str, worker_id: int, real_name: str):
    ...
    await send_live_status(phone_10d, f"📝 Registering {real_name}...", "📝")
    ...
    try:
        ...
        async with session.post(reg_url, json=reg_payload, headers=headers, timeout=API_TIMEOUT+5) as r:
            if r.status == 429:
                await send_live_status(phone_10d, "⏳ Rate limited, retrying...", "⏳")
            ...
            if res.get('message') == 'success':
                await send_live_status(phone_10d, f"✅ Registration success! Sending OTP...", "✅")
                ...
                async with session.post(log_url, json=log_payload, headers=headers, timeout=API_TIMEOUT+5) as lr:
                    if lr.status == 429:
                        await asyncio.sleep(0.1)
                        await number_queue.put((phone_10d, real_name))
                        return
                    ...
                    if lres.get('message') == 'OTP sent to your phone':
                        ...
                        await send_live_status(phone_10d, "📲 OTP Sent! Waiting for OTP...", "📲")
                        print(f"⚡ [W-{worker_id}]: {phone_10d} | {real_name}")
                    else:
                        error_msg = lres.get('message', 'Unknown error')
                        await send_live_status(phone_10d, f"❌ OTP send failed: {error_msg}", "❌")
                        await number_queue.put((phone_10d, real_name))
            else:
                error_msg = res.get('message', 'Unknown error')
                await send_live_status(phone_10d, f"❌ Registration failed: {error_msg}", "❌")
                print(f"❌ Registration failed for {phone_10d}: {error_msg}")
                # Still save number as used to avoid retry loops
                save_used_number(phone_10d)
                
    except Exception as e:
        error_msg = str(e)[:50]
        await send_live_status(phone_10d, f"⚠️ Error: {error_msg}", "⚠️")
        print(f"⚠️ Error for {phone_10d}: {e}")
        # Still save number as used to avoid retry loops
        save_used_number(phone_10d)
...
async def verify_habuild_otp(phone_10d: str, otp: str):
    ...
    if not data:
        await send_live_status(phone_10d, "⏳ Session expired", "⏳")
        return

    await send_live_status(phone_10d, f"🔐 Verifying OTP: `{otp}`...", "🔐")
    ...
    try:
        ...
        async with session.post(url, json=payload, headers=headers, timeout=API_TIMEOUT+5) as r:
            res = await r.json()
            if res.get('message') == 'OTP verified successfully':
                ...
                member_id = member.get('legacy_free_id', 'N/A')
                ...
                if _main_app: 
                    await _main_app.bot.send_message(ADMIN_ID, succ_msg, parse_mode='Markdown')
                    await send_live_status(phone_10d, f"🎉 SUCCESS! Total: {looted_count[0]}", "🎉")
                print(f"🎯 TOTAL REFERRALS: {looted_count[0]}")
                save_used_number(phone_10d)
            else:
                error_msg = res.get('message', 'Unknown error')
                await send_live_status(phone_10d, f"❌ Verify failed: {error_msg}", "❌")
                pending_habuild[phone_10d] = data
    except Exception as e:
        await send_live_status(phone_10d, f"⚠️ Verify error: {str(e)[:30]}", "⚠️")
        pending_habuild[phone_10d] = data
    ...
    if otp_match and ("HABUILD" in sender.upper() or "Habuild" in body):
        ...
        for num in device.get("numbers", []):
            if num in pending_habuild:
                await send_live_status(num, f"📩 OTP `{otp}` received from panel!", "📩")
    ...
    while True:
        try:
            ...
            for num in expired:
                ...
                await send_live_status(num, "⏰ OTP expired, moving on...", "⏰")
        except:
            pass
...
def main():
    print("🚀" * 40)
    print("🔥 ROCKET MODE - FIXED ORIGIN 🔥")
    print("🚀" * 40)
    ...
    async def post_init(application: Application) -> None:
        ...
        print("🎯 ORIGIN FIXED! NOW REGISTRATION WILL WORK!")
