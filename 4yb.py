        ...
        if live_message_id and _main_app:
            try:
                ...
                text = (
                    ...
                    f"Waiting for OTP: {len(pending_habuild)}\n"
                    "━━━━━━━━━━━━━━━━━━\n"
                    f"TOTAL VERIFIED LOOTS: {looted_count[0]}\n\n"
                    "Status: 10s Janitor Engine Active | Auto-Updating..."
                )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    engine_name = get_dynamic_engine_name()
    text = (
        f"{engine_name} STARTED IN UNLIMITED FAST MODE!\n\n"
        "Strict 10-second timeout rule is active. Slow numbers will be automatically dropped. Niche options select karein:"
    )
    keyboard = [
        [InlineKeyboardButton("Live Dash", callback_data='btn_live')],
        [InlineKeyboardButton("Add Panel", callback_data='add_panel'), InlineKeyboardButton("Stats", callback_data='btn_stats')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def cmd_live(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global live_message_id
    if update.effective_chat.id != ADMIN_ID: return
    msg = await update.message.reply_text("Initializing Unlimited Live Dashboard...")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'btn_live':
        ...
        keyboard = [[InlineKeyboardButton("Back to Live Dash", callback_data='btn_live')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if WAITING_FOR_PANEL:
        ...
        if url.startswith("https://") and "firebaseio.com" in url:
            ...
            WAITING_FOR_PANEL = False
            await update.message.reply_text(f"Panel Successfully Added: {new_db_name}\nTotal Panels Ab: {len(DATABASES)}")
        else:
            await update.message.reply_text("Invalid URL. Panel URL 'https://' se start hona chahiye aur usme 'firebaseio.com' hona zaroori hai. Dobara try karein ya /start dabayein.")
    ...
    async def post_init(application: Application) -> None:
        ...
        try:
            ...
            await application.bot.send_message(
                chat_id=ADMIN_ID, 
                text=f"{engine_name} IS ONLINE IN UNLIMITED MODE!\n\nSystem maximum capacity par chal raha hai.",
